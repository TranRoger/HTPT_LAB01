...
.PHONY: all test run clean logs help analyze setup
...
# Run with custom parameters: make run-custom MSGS=200 RATE=50
run-custom:
	python3 src/launch_all.py $(MSGS) $(RATE)

# Run quick test with fewer messages
run-test:
...
# Analyze specific process: make analyze-process PROC=0
analyze-process:
	python3 src/log_analyzer.py $(PROC)
...
# Tail a specific log: make tail PROC=0
tail:
	tail -f logs/process_$(PROC).log
...
log-summary:
	...
	@for i in {0..14}; do \
		if [ -f logs/process_$$i.log ]; then \
			...
			buffered=$$(grep -c "BUFFERED" logs/process_$$i.log || echo "0"); \
...
count:
	...
	@total_sent=$$(grep -rh "→ SENT" logs/ | wc -l); \
	total_received=$$(grep -rh "← RECEIVED" logs/ | wc -l); \
	total_delivered=$$(grep -rh "✓ DELIVERED" logs/ | wc -l); \
	total_buffered=$$(grep -rh "⊕ BUFFERED" logs/ | wc -l); \
...
help:
	...
	@echo "  make run-single PROC=0 - Run single process"
	@echo ""
	@echo "Testing:"
	...
	@echo "  make analyze-process PROC=0 - Analyze specific process"
	@echo "  make stats           - Show statistics"
	@echo "  make count           - Count messages"
	@echo ""
	@echo "Logs:"
	@echo "  make logs            - List log files"
	@echo "  make tail PROC=0     - Tail specific log"
